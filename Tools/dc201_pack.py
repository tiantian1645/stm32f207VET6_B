...
class DC201ErrorCode(Enum):
    eError_Motor_Task_Busy = ("电机任务忙", 0)
    eError_Motor_Heater_Debug = ("上加热体电机错误调试", 1)
    eError_Motor_Heater_Timeout_Up = ("上加热体电机运动超时 上升方向", 2)
    eError_Motor_Heater_Timeout_Down = ("上加热体电机运动超时 下降方向", 3)
    eError_Motor_Heater_Status_Warui = ("上加热体电机驱动状态异常", 4)
    eError_Motor_White_Debug = ("白板电机错误调试", 5)
    eError_Motor_White_Timeout_PD = ("白板电机运动超时 PD方向", 6)
    eError_Motor_White_Timeout_WH = ("白板电机运动超时 白物质方向", 7)
    eError_Motor_White_Status_Warui = ("白板电机驱动状态异常", 8)
    eError_Motor_Tray_Debug = ("托盘电机错误调试", 9)
    eError_Motor_Tray_Busy = ("托盘电机驱动忙", 10)
    eError_Motor_Tray_Timeout = ("托盘电机运动超时", 11)
    eError_Motor_Tray_Status_Warui = ("托盘电机驱动状态异常", 12)
    eError_Motor_Scan_Debug = ("扫码电机错误调试", 13)
    eError_Motor_Scan_Busy = ("扫码电机驱动忙", 14)
    eError_Motor_Scan_Timeout = ("扫码电机运动超时", 15)
    eError_Motor_Scan_Status_Warui = ("扫码电机驱动状态异常", 16)
    eError_Temperature_Top_Abnormal = ("上加热体温度异常", 17)
    eError_Temperature_Top_TooHigh = ("上加热体温度过高", 18)
    eError_Temperature_Top_TooLow = ("上加热体温度过低", 19)
    eError_Temperature_Btm_Abnormal = ("下加热体温度异常", 20)
    eError_Temperature_Btm_TooHigh = ("下加热体温度过高", 21)
    eError_Temperature_Btm_TooLow = ("下加热体温度过低", 22)
    eError_Scan_Debug = ("扫码枪错误调试", 23)
    eError_Scan_Connect_Timeout = ("扫码枪通讯超时", 24)
    eError_Scan_Config_Failed = ("扫码枪配置失败", 25)
    eError_Storge_Task_Busy = ("存储任务忙", 26)
    eError_ID_Card_Deal_Param = ("ID Code卡操作参数异常", 27)
    eError_ID_Card_Read_Failed = ("ID Code卡读取失败", 28)
    eError_ID_Card_Write_Failed = ("ID Code卡写入失败", 29)
    eError_ID_Card_Not_Insert = ("ID Code卡未插卡", 30)
    eError_Out_Flash_Deal_Param = ("外部Flash操作参数异常", 31)
    eError_Out_Flash_Read_Failed = ("外部Flash读取失败", 32)
    eError_Out_Flash_Write_Failed = ("外部Flash写入失败", 33)
    eError_Out_Flash_Unknow = ("外部Flash型号无法识别", 34)
    eError_Out_Flash_Storge_Param_Out_Of_Range = ("外部Flash存储参数越限", 35)
    eError_Comm_Main_Busy = ("主串口发送忙", 36)
    eError_Comm_Main_Send_Failed = ("主串口发送失败", 37)
    eError_Comm_Main_Not_ACK = ("主串口没有收到ACK", 38)
    eError_Comm_Main_Wrong_ID = ("主串口异常ID", 39)
    eError_Comm_Main_Unknow_CMD = ("主串口异常功能码", 40)
    eError_Comm_Main_Param_Error = ("主串口报文参数异常", 41)
    eError_Comm_Data_Busy = ("采样板查串口发送忙", 42)
    eError_Comm_Data_Send_Failed = ("采样板查串口发送失败", 43)
    eError_Comm_Data_Not_ACK = ("采样板查串口没有收到ACK", 44)
    eError_Comm_Data_Wrong_ID = ("采样板查串口异常ID", 45)
    eError_Comm_Data_Unknow_CMD = ("采样板查串口异常功能码", 46)
    eError_Comm_Data_Param_Error = ("采样板查串口报文参数异常", 47)
    eError_Comm_Out_Busy = ("外串口发送忙", 48)
    eError_Comm_Out_Send_Failed = ("外串口发送失败", 49)
    eError_Comm_Out_Not_ACK = ("外串口没有收到ACK", 50)
    eError_Comm_Out_Wrong_ID = ("外串口异常ID", 51)
    eError_Comm_Out_Unknow_CMD = ("外串口异常功能码", 52)
    eError_Comm_Out_Param_Error = ("外串口报文参数异常", 53)
