import re
import struct
import loguru
import codecs

logger = loguru.logger.bind(name=__name__)
CRC8Table = [
    ,
]


def crc8(pack):
    result = 0
    for p in pack:
        result = CRC8Table[result ^ p]
    return struct.pack(">B", result)


def str2Bytes(s: str):
    s_hex = re.compile("[^a-fA-F0-9]").sub("", s)
    if len(s_hex) % 2 == 1:
        logger.warning("last byte will be add 0 in font")
        s_hex = "{}0{}".format(s_hex[:-1], s_hex[-1])
    return bytes.fromhex(s_hex)


def bytesPuttyPrint(b: bytes):
    st = codecs.encode(b, "hex").decode("utf-8").upper()
    it = iter(st)
    result_str = " ".join(a + b for a, b in zip(it, it))
    return result_str


def bytes2Int(b: bytes, index: int = 0, length: int = 0, signed=False):
    if index == 0 and length == 0:
        return int.from_bytes(b, byteorder="big", signed=signed)
    else:
        return int.from_bytes(b[index : index + length], byteorder="big", signed=signed)


def bytess2Float(b: bytes, model="DCBA"):
    if len(b) != 4:
        logger.error("convert bytes only support 4 bytes")
        raise ValueError("Bytes length error")
    model = model.upper()
    if model.upper() not in ("ABCD", "DCBA", "BADC", "CDAB"):
        logger.warning('model should choice from ("ABCD", "DCBA", "BADC", "CDAB") now use "DCBA"')
        model = "DCBA"
    if model == "DCBA":
        return struct.unpack("f", b)[0]
    elif model == "ABCD":
        return struct.unpack(">f", b)[0]
    elif model == "BADC":
        return struct.unpack("f", b[2:] + b[:2])[0]
    else:
        return struct.unpack(">f", b[2:] + b[:2])[0]
