...
/* Private variables ---------------------------------------------------------*/
/* @note Reflected 0xEDB88320: &1 >> [128] = poly , [ 8] = poly >> 8 VALID */
...
/* Private constants ---------------------------------------------------------*/
#define INNATE_FLASH_SECTOR_TEMP (FLASH_SECTOR_11)
#define INNATE_FLASH_SECTOR_REAL (FLASH_SECTOR_0)
...
/**
 * @brief  CRC32校验码生成
 * @note   https://github.com/Michaelangel007/crc32
 * @note   reverse polynomial
 * @note   https://github.com/Michaelangel007/crc32/blob/54ea67ad99b2eb5d70b54a5bf67f06eff614c7e6/src/crc32.h#L182
 * @param  buf 数据指针
 * @param  len 数据长度
 * @retval CRC32校验码
 **/
...
/**
 * @brief  擦除内部Flash
 * @param  flash_sector 区块索引  FLASH_SECTOR_X
 * @param  num 区块数量
 * @retval 0 擦除成功 1 擦除失败
 **/
uint8_t Innate_Flash_Erase(uint32_t flash_sector,  uint8_t num)
{
    FLASH_EraseInitTypeDef fer = {0};
    uint32_t flash_srror = 0;
    ...
    do {
        fer.TypeErase = FLASH_TYPEERASE_SECTORS;
        fer.Banks = FLASH_BANK_1;
        fer.Sector = flash_sector;
        fer.NbSectors = num;
        fer.VoltageRange = FLASH_VOLTAGE_RANGE_3;
        status = HAL_FLASHEx_Erase(&fer, &flash_srror);
        if (status == HAL_OK) {
            cnt = 0;
            break;
        }
...
{
    return Innate_Flash_Erase(INNATE_FLASH_SECTOR_TEMP, 1);
}

uint8_t Innate_Flash_Erase_Real(void)
{
    return Innate_Flash_Erase(INNATE_FLASH_SECTOR_REAL, 3);
}

/**
 * @brief  读取内部Flash地址值对应数据
 * @param  addr 地址
 * @param  type 数据类型
 * @retval 地址上对应值
 **/
 ...
 * @brief  写入内部Flash
 * @param  addr 起始地址
 * @param  uint8_t *pBuffer, uint8_t length 数据描述 字节元素
...
uint8_t Innate_Flash_Write(uint32_t addr, uint8_t * pBuffer, uint16_t length)
{
    ...
    if (length % 4 == 0) { /* 长度能被8整除 按 word 模式写入 */
        for (i = 0; i < length / 4; ++i) {
            cnt = 0;
            do {
                data = (pBuffer[i * 4 + 3] << 24) + (pBuffer[i * 4 + 2] << 16) + (pBuffer[i * 4 + 1] << 8) + (pBuffer[i * 4 + 0] << 0);
                status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, addr, data);
                if (status == HAL_OK) {
                    if (Innate_Flash_Get_Data_By_Addr(addr, 4) == data) {
                        addr += 4;
                        cnt = 0;
                        break;
                    }
        ...
        for (i = 0; i < length; ++i) {
            cnt = 0;
            do {
                status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, addr, pBuffer[i]);
                if (status == HAL_OK) {
                    if (Innate_Flash_Get_Data_By_Addr(addr, 1) == pBuffer[i]) {
                        addr += 1;
                        cnt = 0;
                        break;
                    }
...
uint8_t Innate_Flash_Copy(uint32_t dst, uint32_t src, uint32_t num)
{
    union {
        uint32_t u32;
        uint8_t u8s[4];
    ...
    for (i = 0; i < num / 4; ++i) {
        data.u32 = Innate_Flash_Get_Data_By_Addr(src + i * 4, 4);
        if (Innate_Flash_Write(dst + i * 4, data.u8s, 4)) {
            return 1;
        }
    }
    return 0;
...
uint8_t Innate_Flash_Dump(uint16_t total, uint32_t check_sum)
{
    ...
    for (i = 0; i < total; ++i) {
        data = Innate_Flash_Get_Data_By_Addr(INNATE_FLASH_ADDR_TEMP + i, 1);
    ...
    if (Innate_Flash_Erase_Real()) {
        return 2;
    }
    if (Innate_Flash_Copy(INNATE_FLASH_ADDR_REAL, INNATE_FLASH_ADDR_TEMP, total)) {
        HAL_FLASH_Lock();
        return 3;
    }
    HAL_FLASH_Lock();
