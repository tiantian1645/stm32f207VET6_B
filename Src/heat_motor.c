...
uint8_t heat_Motor_PWM_Gen_Up(void)
{
    ...
    gHeat_Motor_SRC_Buffer[0] =
        (HEAT_MOTOR_PCS_MAX - HEAT_MOTOR_PCS_MIN > HEAT_MOTOR_PCS_GAP * (total % HEAT_MOTOR_PCS_SUM)) ? (HEAT_MOTOR_PCS_MAX - HEAT_MOTOR_PCS_GAP * (total % HEAT_MOTOR_PCS_SUM)) : (HEAT_MOTOR_PCS_MAX); /* 周期长度 */
...
uint8_t heat_Motor_PWM_Gen_Down(void)
{
    ...
    if (cnt >= HEAT_MOTOR_PCS_SUM) { /* 停止输出 */
        ...
        gHeat_Motor_SRC_Buffer[0] = (HEAT_MOTOR_PCS_MAX - HEAT_MOTOR_PCS_MIN > HEAT_MOTOR_PCS_GAP * cnt) ? (HEAT_MOTOR_PCS_MAX - HEAT_MOTOR_PCS_GAP * cnt) : (HEAT_MOTOR_PCS_MIN); /* 周期长度 */
        gHeat_Motor_SRC_Buffer[1] = HEAT_MOTOR_PCS_UNT;                                                                                         /* 重复次数 */
        gHeat_Motor_SRC_Buffer[2] = (gHeat_Motor_SRC_Buffer[0] + 1) / 2; /* 占空比 默认50% */
        /* burst模式修改时基单元 */
        HAL_TIM_DMABurst_WriteStart(&htim1, TIM_DMABASE_ARR, TIM_DMA_UPDATE, (uint32_t *)gHeat_Motor_SRC_Buffer, TIM_DMABURSTLENGTH_3TRANSFERS);
