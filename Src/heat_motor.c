...
{
    return (gHeat_Motor_Position_Get() != 0xFFFFFFFF) && (gHeat_Motor_Position_Get() > PWM_PCS_SUM * PWM_PCS_UNT * 80 / 100);
}
...
uint8_t heat_Motor_PWM_Gen_Down(void)
{
    ...
    if (cnt >= PWM_PCS_SUM) { /* 停止输出 */
        ...
        gHeat_Motor_SRC_Buffer[0] = (PWM_PCS_MAX - PWM_PCS_MIN > PWM_PCS_GAP * cnt) ? (PWM_PCS_MAX - PWM_PCS_GAP * cnt) : (PWM_PCS_MIN); /* 周期长度 */
        gHeat_Motor_SRC_Buffer[1] = PWM_PCS_UNT;                                                                                         /* 重复次数 */
        gHeat_Motor_SRC_Buffer[2] = (gHeat_Motor_SRC_Buffer[0] + 1) / 2; /* 占空比 默认50% */
        /* burst模式修改时基单元 */
        HAL_TIM_DMABurst_WriteStart(&htim1, TIM_DMABASE_ARR, TIM_DMA_UPDATE, (uint32_t *)gHeat_Motor_SRC_Buffer, TIM_DMABURSTLENGTH_3TRANSFERS);
